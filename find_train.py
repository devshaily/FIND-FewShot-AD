...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        out = model(sample["rgb_norm"].to(DEVICE), sample["normal_norm"].to(DEVICE))
        print("Teacher intra shapes:", [list(f.shape) for f in out["intra_f2d"]])
        print("Student intra shapes:", [list(f.shape) for f in out["intra_s2d_feats"]])
        print("Cross feat shapes   :", list(out["cross_f2d"].shape), list(out["cross_f3d"].shape))
        print("Recon 2D shape      :", list(out["recon_2d"].shape))
    ...
    evaluate(model, test_loader)
     ...
     # ── Save anomaly maps ─────────────────── ← ADD this
    save_anomaly_maps_for_official_eval(
        model, test_loader, "anomaly_maps", CATEGORY
    )

    # ── Visualize ─────────────────────────────
    visualize_results(model, test_loader, num_samples=8, save_dir=f"visualizations/{CATEGORY}")
