...
EMBED_DIM = 768
ALPHA     = 0.8                         # few-shot weight: more cross-modal
EPOCHS    = 50
LR        = 5e-5
BATCH     = 2
CATEGORY  = "dowel"
DATA_ROOT = r"C:\Users\flexi\Downloads\mvtec_3d_anomaly_detection"
...
def xyz_to_surface_normal(xyz: np.ndarray) -> np.ndarray:
    """
    Convert an H×W×3 XYZ map (from MVTec3D .tiff) to an H×W×3 surface normal map.
    Uses cross product of neighbouring pixel vectors.
    Returns float32 array in [0, 1] range (encoded as RGB).
    """
    H, W, _ = xyz.shape
    ...
    dxdu = np.zeros_like(xyz)
    dxdv = np.zeros_like(xyz)

    dxdu[:, :-1] = xyz[:, 1:] - xyz[:, :-1]   # horizontal gradient
    dxdv[:-1, :] = xyz[1:, :] - xyz[:-1, :]   # vertical gradient
...
def intra_modal_loss(out: dict, rgb: torch.Tensor, normal: torch.Tensor) -> torch.Tensor:
    """
    Eq. (3) from paper:
      - Cosine loss at each intermediate layer between student & teacher features
      - MSE loss at pixel level between reconstruction and original image
    
    Note: student intermediate features are at lower resolutions (56, 112).
    We resize teacher features to match for the cosine loss.
    """
    loss = 0.0
    count = 0

    # ── Intermediate layer cosine losses ─────────────────
    # intra_f2d has 3 teacher features. Student has 2 intermediate outputs.
    # We compare the last 2 teacher features with student intermediates.
    teacher_2d_for_intra = out["intra_f2d"][1:]   # layers 6, 8 (skip first)
    teacher_3d_for_intra = out["intra_f3d"][1:]

    for t_feat, s_feat in zip(teacher_2d_for_intra, out["intra_s2d_feats"]):
        # Resize teacher to match student spatial size if needed
...
def compute_anomaly_map(model: FIND, batch: dict) -> np.ndarray:
    ...
    intra_maps_2d = []
    for t_feat, s_feat in zip(out["intra_f2d"][1:], out["intra_s2d_feats"]):
        # Resize teacher to match student spatial size before comparison
        if t_feat.shape[-1] != s_feat.shape[-1]:
            t_feat = F.interpolate(t_feat, size=s_feat.shape[-2:], mode="bilinear", align_corners=False)
    ...
    intra_maps_3d = []
    for t_feat, s_feat in zip(out["intra_f3d"][1:], out["intra_s3d_feats"]):
        # Resize teacher to match student spatial size before comparison
        if t_feat.shape[-1] != s_feat.shape[-1]:
            t_feat = F.interpolate(t_feat, size=s_feat.shape[-2:], mode="bilinear", align_corners=False)
...
if __name__ == "__main__":
    ...
    print(f"K-shot : {None}  (change K_SHOT variable as needed)")

    K_SHOT = None   # set to None for full-shot

    # ── Datasets ─────────────────────────────
    train_dataset = MVTec3DDataset(
        root=DATA_ROOT, category=CATEGORY, split="train", k_shot=K_SHOT
    )
    ...
    ckpt_path = f"checkpoints/find_{CATEGORY}_k{K_SHOT}.pt"
    torch.save({
    ...
    # ── Visualize ─────────────────────────────
    visualize_results(model, test_loader, num_samples=8, save_dir="visualizations")
...
#     model = FIND().to(DEVICE)
#     ckpt = torch.load(r"D:\checkpoints\find_bagel_k10.pt", map_location=DEVICE)
