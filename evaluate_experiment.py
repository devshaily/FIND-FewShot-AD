...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir):
    ...
    for subdir in listdir(str(test_dir)):
        ...
        if not path.exists(gt_dir):    # ← add this
            continue
...
def main():
    ...
    DATASET_BASE_DIR    = r"D:\Deep_Learning\FIND\mvtec_3d_anomaly_detection"
    ANOMALY_MAPS_DIR    = r"D:\Deep_Learning\FIND\anomaly_maps"
    OUTPUT_DIR          = r"D:\Deep_Learning\FIND\metrics\pro30"
    PRO_LIMIT           = 0.3        # change to 0.01 for AUPRO@1%
    EVALUATED_OBJECTS   = ["dowel"]  # or list all 10 for full eval
    ...
    for obj in EVALUATED_OBJECTS:
        ...
        au_pro, au_roc, pro_curve, roc_curve = calculate_au_pro_au_roc(
            gt_filenames,
            prediction_filenames,
            PRO_LIMIT
        )
    ...
    evaluation_dict['mean_au_pro'] = mean_au_pro / len(EVALUATED_OBJECTS)
    evaluation_dict['mean_au_roc'] = mean_au_roc / len(EVALUATED_OBJECTS)
